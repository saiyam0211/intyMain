...
const ScheduleMeeting = ({ contact, isOpen, onClose, contactType = "designer" }) => {
  console.log('ScheduleMeeting rendering:', { isOpen, contactName: contact?.name, contactType });
  ...
  const handleCreateMeeting = async (e) => {
    ...
    try {
      ...
      const contactTitle = contactType === 'designer' ? 'Interior Designer' : 'Craftsman';
      
      // Create the meeting details
      const meetingDetails = {
        summary: `Meeting with ${contact.name} (${contactTitle})`,
        description: `${meetingType === 'virtual' ? 'Virtual' : 'In-person'} consultation meeting between ${user.primaryEmailAddress?.emailAddress} and ${contact.name} (${contactTitle}).`,
        location: meetingType === 'virtual' ? 'Google Meet' : contact.location || 'To be determined',
        startTime,
        endTime,
        userEmail: user.primaryEmailAddress?.emailAddress,
        contactEmail: contact.email || 'info@inty.co',
      ...
      const add = encodeURIComponent(`${meetingDetails.userEmail},${meetingDetails.contactEmail}`);
      ...
      >
        <h2 className="text-lg font-semibold text-gray-800 mb-4">
          ...
          <span className="text-xs ml-2 text-gray-500 font-normal">
            ({contactType === 'designer' ? 'Interior Designer' : 'Craftsman'})
          </span>
        </h2>
